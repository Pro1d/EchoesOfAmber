...
func _physics_process(_delta: float) -> void:
	if self.atraction_point != null:
		...
		var target := atraction_point.global_position
		var pull_vector := (target - self.global_position).normalized()
		var tangent := Vector2(-pull_vector.y, pull_vector.x)
		var max_dist := 96.0
		var distance_ratio := clampf((target - self.global_position).length() / max_dist, 0.0, 1.0)
